 ...
 * @returns true
 */
export function startProcessing(formGroup: FormGroup | undefined, dialog: MatDialog): boolean {
  if (formGroup) {
    formGroup.disable();
  }
  dialog.open(DialogMessageComponent, new DialogMessage(DialogMessageTypes.LOADING, DialogMessageTypes.LOADING_TITLE, DialogMessageTypes.LOADING_REQUEST_MESSAGE).getDialogJson());
  return true;
}
 ...
 * @returns 
 */
export function endProcessing(formGroup: FormGroup | undefined, dialog: MatDialog): boolean {
  if (formGroup) {
    formGroup.enable();
  }
  dialog.closeAll();
